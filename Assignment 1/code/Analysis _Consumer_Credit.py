...
# Parameter Tuning
params_name={'max_depth': np.arange(4, 20),
...
plt.title('Learning Curves: Decision Tree ccp_alpha adjusted for Consumer Credit')
plt.xlabel('Training Sizes')
plt.ylabel('Score')
plt.legend(loc='best')
plt.grid()
plt.savefig(fig_path + 'dtree_ccp_alpha_adjusted_for_consumer.png')
plt.clf()
...
nn = MLPClassifier(random_state=id, max_iter=1500)
params_name = {'alpha': [0.001, 0.01, 0.1],
...
# Parameter Tuning
params_name = {'n_estimators': range(70, 80, 1)}
